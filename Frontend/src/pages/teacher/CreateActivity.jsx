...
const CreateActivity = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (totalWeight !== 100 && formData.questions.length > 0) {
  ...
  const handleUploadMarks = async (e, activityId) => {
    const file = e.target.files[0];
    if (!file) return;

    try {
      setUploadLoading(true);
      await uploadActivityMarks(activityId, file);
      setStatus('success');
      await handleViewSubmissions(activityId);
      setTimeout(() => setStatus('idle'), 3000);
    } catch (err) {
      setError(err.message);
      setStatus('error');
  ...
  const handleSaveEditedMarks = async () => {
    ...
    try {
      setEditSaving(true);
      await editActivityMarks(
        submissionsModal.data.activity._id,
        editingSubmission._id,
        editFormData
      );
      setStatus('success');
      await handleViewSubmissions(submissionsModal.data.activity._id);
        ...
        <main className="flex-1 overflow-y-auto p-6 lg:p-8 relative">
          ...
          <div className="flex flex-col xl:flex-row gap-10">
            <div className="w-full xl:w-1/2 space-y-6">
              <h3 className="text-3xl font-black uppercase flex items-center gap-3 mb-6 bg-white border-6 border-black p-4 inline-block" style={{ boxShadow: '6px 6px 0px #000' }}>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="bg-white border-6 border-black p-5" style={{ boxShadow: '8px 8px 0px #000' }}>
                  ...
                  <input 
                    ...
                    className="w-full border-4 border-black p-4 font-bold uppercase focus:bg-[#00FFFF] outline-none transition-all text-lg"
                ...
                <div className="bg-white border-6 border-black p-5" style={{ boxShadow: '8px 8px 0px #000' }}>
                  ...
                  <textarea 
                    ...
                    className="w-full border-4 border-black p-4 font-bold focus:bg-[#00FF00] outline-none transition-all text-base"
                ...
                <div className="bg-white border-6 border-black p-5" style={{ boxShadow: '8px 8px 0px #000' }}>
                  ...
                  <div className="space-y-3 max-h-56 overflow-y-auto pr-2">
                    {classes.map(cls => (
                      <label key={cls._id} className="flex items-center gap-4 cursor-pointer hover:bg-[#FFEB3B] p-3 transition-all border-2 border-transparent hover:border-black">
                        <input 
                          type="checkbox"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-white border-6 border-black p-5" style={{ boxShadow: '8px 8px 0px #000' }}>
                    ...
                    <input 
                      required
                      type="date" 
                      className="w-full border-3 border-black p-3 font-bold focus:bg-[#FF00FF] focus:text-white outline-none transition-all"
                  ...
                  <div className="bg-white border-6 border-black p-5" style={{ boxShadow: '8px 8px 0px #000' }}>
                    ...
                    <input 
                      required
                      type="number" 
                      className="w-full border-3 border-black p-3 font-bold text-lg"
                ...
                <div className="bg-white border-6 border-black p-6" style={{ boxShadow: '8px 8px 0px #000' }}>
                  <div className="flex justify-between items-center mb-6">
                    ...
                    <button 
                      ...
                      className="bg-[#00FF00] border-4 border-black px-4 py-2 font-black uppercase text-sm flex items-center gap-2 hover:-translate-y-1 active:translate-y-0 transition-all"
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
                    {formData.questions.map((question, idx) => (
                      <motion.div 
                        ...
                        className="bg-[#F8F9FA] border-4 border-black p-4 hover:border-[#FF00FF] hover:bg-[#FFE5FF] transition-all"
                      >
                         <div className="flex flex-col gap-3">
                            <div className="flex gap-3">
                              ...
                              <input 
                                ...
                                className="flex-1 border-b-4 border-black bg-transparent p-2 font-bold focus:outline-none focus:bg-[#00FFFF]"
                            ...
                            <div className="flex items-center justify-between pl-13">
                              <div className="flex items-center gap-3">
                                <label className="text-xs font-black uppercase">Weight %</label>
                                <div className="flex items-center border-3 border-black bg-white">
                                  <input 
                                    type="number"
                                    placeholder="0"
                                    className="w-16 p-2 font-black text-center outline-none text-lg"
                                    value={question.weight}
                                    onChange={(e) => handleQuestionChange(question.id, 'weight', e.target.value)}
                                  />
                                  <span className="px-3 font-black border-l-3 border-black bg-[#FFEB3B] text-lg">%</span>
                                </div>
                              </div>
                              ...
                              <button 
                                ...
                                className="text-red-500 hover:text-red-700 hover:bg-red-100 p-2 transition-all font-black"
                              >
                                <Trash2 size={20} />
                              </button>
                            </div>
                  ...
                  <div className="mt-6 p-4 bg-black text-white flex justify-between items-center font-black text-lg border-4 border-white">
                    ...
                    <span className={`text-2xl ${
                      formData.questions.reduce((sum, q) => sum + (q.weight || 0), 0) === 100 
                        ? 'text-[#00FF00]' 
                        : 'text-[#FF0000]'
                    }`}>
                      {formData.questions.reduce((sum, q) => sum + (q.weight || 0), 0)}%
                    </span>
                  </div>
                ...
                <button 
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-black text-white border-6 border-black p-6 text-xl font-black uppercase flex items-center justify-center gap-3 hover:bg-[#FF00FF] active:scale-95 transition-all"
            ...
            <div className="w-full xl:w-1/2">
               <h3 className="text-3xl font-black uppercase flex items-center gap-3 mb-6 bg-[#00FF00] border-6 border-black p-4 inline-block" style={{ boxShadow: '6px 6px 0px #000' }}>
                <Layout size={32} /> Activities
              </h3>

              <div className="space-y-4">
                {listLoading ? (
                  <div className="flex flex-col items-center py-24 bg-white border-6 border-black" style={{ boxShadow: '8px 8px 0px #000' }}>
                    <Loader2 className="animate-spin mb-4 text-[#FF00FF]" size={56} />
                    <p className="font-black uppercase text-xl">Loading...</p>
                  </div>
                ) : activities.length === 0 ? (
                  <div className="bg-white border-6 border-black p-12 text-center" style={{ boxShadow: '8px 8px 0px #000' }}>
                  ...
                  activities.map((activity, idx) => (
                    <motion.div 
                      ...
                      className="bg-white border-6 border-black p-6 hover:shadow-2xl transition-all group"
                      style={{ boxShadow: '8px 8px 0px #000' }}
                    >
                      <div className="flex justify-between items-start mb-3">
                        <h4 className="text-2xl font-black uppercase flex-1 pr-3 text-[#000]">{activity.title}</h4>
                        <span className={`text-xs font-black uppercase px-3 py-1 border-2 border-black whitespace-nowrap ${
                          activity.status === 'Active' ? 'bg-[#00FF00] text-black' : 'bg-gray-400'
                        }`}>
                          {activity.status}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-wrap gap-2 text-xs font-black uppercase mb-5">
                        ...
                        <span className="bg-[#FFEB3B] border-2 border-black px-3 py-1 flex items-center gap-1 font-bold">
                      ...
                      <div className="mt-5 pt-5 border-t-4 border-black flex gap-2 flex-wrap">
                        <button 
                          ...
                          className="flex-1 min-w-24 bg-white border-4 border-black p-3 font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-[#FFEB3B] active:scale-95 transition-all"
                          style={{ boxShadow: '4px 4px 0px #000' }}
                        >
                        ...
                        <button 
                          ...
                          className="flex-1 min-w-24 bg-[#00FF00] border-4 border-black p-3 font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-[#00FFFF] active:scale-95 transition-all"
                          style={{ boxShadow: '4px 4px 0px #000' }}
                        >
                          <Eye size={16} /> View
                        </button>
                        <button 
                          ...
                          className="flex-1 min-w-24 bg-black text-white border-4 border-black p-3 font-black uppercase text-xs flex items-center justify-center gap-2 hover:bg-[#FF00FF] active:scale-95 transition-all"
                          style={{ boxShadow: '4px 4px 0px #000' }}
                        >
                      ...
                      <AnimatePresence>
                        {evaluatingId === activity._id && (
                          ...
                          >
                            <div className="bg-[#00FF00] border-4 border-black p-5">
            ...
            {submissionsModal.open && submissionsModal.data && (
              ...
              >
                <motion.div 
                  ...
                  className="bg-white border-8 border-black max-w-5xl w-full max-h-[92vh] overflow-y-auto"
                  style={{ boxShadow: '20px 20px 0px rgba(0,0,0,0.5)' }}
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="sticky top-0 bg-gradient-to-r from-[#FFEB3B] to-[#00FFFF] border-b-8 border-black p-6 flex justify-between items-center">
                    <h3 className="text-3xl font-black uppercase text-black">{submissionsModal.data.activity.title}</h3>
                    <button 
                      onClick={() => setSubmissionsModal({ open: false, activity: null, data: null })}
                  ...
                  {editingSubmission ? (
                    <div className="p-8 space-y-6">
                      <div className="bg-gradient-to-r from-[#FFEB3B] to-[#00FFFF] border-6 border-black p-6">
                      ...
                      <div className="bg-white border-6 border-black p-6 space-y-5">
                        <div>
                          ...
                          <div className="border-6 border-black bg-white overflow-hidden">
                            ...
                            <div className="divide-y-3 divide-black">
                              {editingSubmission.rubrics.map((criterion, idx) => (
                                <div key={criterion} className="flex items-center hover:bg-[#FFFACD] transition-all">
                                  ...
                                  <div className="w-32 p-4">
                                    <input 
                                      ...
                                      className="w-full border-4 border-black p-2 font-black text-lg text-center focus:bg-[#00FFFF] focus:text-black"
                                    />
                                  </div>
                                </div>
                              ))}
                        ...
                        <div className="bg-black text-white border-4 border-white p-4 flex justify-between items-center font-black text-lg">
                          ...
                          <span className="bg-[#FF00FF] px-6 py-2 text-2xl">
                      ...
                      <div className="bg-white border-6 border-black p-6">
                        ...
                        <textarea 
                          ...
                          className="w-full border-4 border-black p-4 font-bold text-base focus:bg-[#00FF00] focus:text-black outline-none transition-all"
                          placeholder="Add feedback for the student..."
                        />
                      </div>

                      {editingSubmission.editHistory && editingSubmission.editHistory.length > 0 && (
                        <div className="border-6 border-black p-6 bg-white">
                              ...
                              <div key={idx} className="border-l-6 border-[#FF00FF] bg-[#F8F9FA] p-4 border-4 border-black">
                                ...
                                <div className="mt-3 space-y-2">
                                  {Object.entries(edit.changes || {}).map(([key, val]) => (
                                    <div key={key} className="bg-[#FFEB3B] p-2 border-2 border-black font-bold text-sm">
                      ...
                      <div className="flex gap-4">
                        <button 
                          ...
                          disabled={editSaving}
                          className="flex-1 bg-[#00FF00] border-6 border-black p-4 font-black uppercase text-lg flex items-center justify-center gap-3 hover:shadow-lg active:scale-95 transition-all"
                        ...
                        <button 
                          ...
                          className="flex-1 bg-black text-white border-6 border-black p-4 font-black uppercase text-lg hover:bg-red-600 active:scale-95 transition-all"
                          style={{ boxShadow: '6px 6px 0px #000' }}
                        >
                          CANCEL
                        </button>
                          ...
                          >
                            ...
                            <div className="w-40 min-w-40 p-6 flex gap-2 justify-center">
                              <button 
                                ...
                                className="bg-[#00FFFF] border-3 border-black px-3 py-2 text-xs font-black uppercase hover:bg-[#FF00FF] hover:text-white active:scale-90 transition-all"
