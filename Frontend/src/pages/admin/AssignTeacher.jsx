...
const AssignTeacher = () => {
  ...
  useEffect(() => {
    ...
    if (!token || !userString) { navigate('/login'); return; }
    const user = JSON.parse(userString);
    if (user.role && user.role.toLowerCase() !== 'admin') { navigate('/'); return; }
    setAdminData(user);
  }, [navigate]);
          ...
          >
            <div className="bg-white border-8 border-black p-8 md:p-10" style={{ boxShadow: '16px 16px 0px #000' }}>
              <div className="flex items-center gap-4 mb-2">
                ...
                <h1 className="text-4xl md:text-5xl font-black uppercase tracking-tight">Assign Teacher</h1>
              </div>
                  ...
                  <div>
                    ...
                    <div className="relative group">
                      ...
                      <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                        className="w-full bg-[#f8f8f8] border-4 border-black pl-12 pr-4 py-3 text-lg font-bold focus:outline-none focus:bg-white focus:-translate-y-1 focus:translate-x-1 transition-all"
